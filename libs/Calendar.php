...
class Calendar
{
    ...
    public function loadEventData($event_id, $event_date)
    {
        ...
        $response = wp_remote_get('https://churchsocialapp.com/api/events/'.$event_id.'/'.$event_date, [
            'headers' => [
    ...
    public function loadEventsData()
    {
        ...
        $response = wp_remote_get('https://churchsocialapp.com/api/events'.(isset($_GET['month']) ? '?month='.$_GET['month'] : ''), [
            'headers' => [
                'Authorization' => $this->api_key,
            ],
        ]);
